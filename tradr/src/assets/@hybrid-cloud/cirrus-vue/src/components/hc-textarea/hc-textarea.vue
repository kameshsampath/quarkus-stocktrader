...
<template>
  <fieldset class="form-item" :class="{ 'has-error': hcStatus == 'error' }">
    ...
    <textarea
      :id="hcId"
      class="input input--textarea input--primary"
      ref="input"
      :disabled="disabled"
      :name="name"
      :value="value"
      ...
      <hc-icon v-show="hcStatus == 'error'" :href="hcIconHref" class="icon--warn error--textarea"></hc-icon>
  </fieldset>
</template>
